...
setup_website() {
    ...
    mkdir -p /insaflu_web/INSaFLU/env && mv /tmp_install/configs/insaflu.env /insaflu_web/INSaFLU/env/insaflu.env && chown -R ${APP_USER}:slurm * && mkdir /var/log/insaFlu && chown -R ${APP_USER}:slurm /var/log/insaFlu
    if [ $? -ne 0 ]; then
