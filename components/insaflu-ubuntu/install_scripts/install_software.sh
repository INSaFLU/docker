...
git clone --branch v2.5.2 https://github.com/pezmaster31/bamtools.git && cd bamtools && mkdir build && cd build && cmake .. && make
if [ $? -ne 0 ]; then
...
### add blast+ to path
#echo 'export PATH="/software/blast+/2.16.0/ncbi-blast-2.16.0+-src/c++/ReleaseMT/bin:$PATH"' >> /root/.bashrc
if [ $? -ne 0 ]; then
